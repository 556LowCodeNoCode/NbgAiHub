...
- **RSS source list — live in production with 5 feeds, two source groups** (revisable in `config/rss-sources.json` without code change). Source-group routing in `pipeline/src/triage.ts` SYSTEM_PROMPT:
  - **Reddit group** (`type: "reddit-json"`; strict — 4 ACCEPT categories: tips/tricks, field reports, platform news, professional/enterprise; engagement floor at fetch time per DECISIONS 2026-05-21 — drop stickies, `score>=50`, `num_comments>=10`):
    1. r/ClaudeAI — `https://www.reddit.com/r/ClaudeAI/new.json?limit=25`
    2. r/ClaudeCode — `https://www.reddit.com/r/ClaudeCode/new.json?limit=25`
